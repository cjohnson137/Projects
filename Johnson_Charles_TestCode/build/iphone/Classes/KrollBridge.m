...
NSString * Johnson_Charles_TestCode$ModuleRequireFormat = @"(function(exports){"
...
@implementation Johnson_Charles_TestCodeObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_johnson_charles_testcode);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create Johnson_Charles_TestCode global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Johnson_Charles_TestCode" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_johnson_charles_testcode = [[Johnson_Charles_TestCodeObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_johnson_charles_testcode];
	
	NSString *_johnson_charles_testcodeNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _johnson_charles_testcodeNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_johnson_charles_testcode valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:Johnson_Charles_TestCode$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.johnson_charles_testcode.kroll" 
