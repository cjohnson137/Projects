...
//Rare exceptions to indicate a bug in the _johnson_charles_project4 code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_johnson_charles_project4 = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_johnson_charles_project4]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.johnson_charles_project4.module"
